...
fn calc_wp_bits_8k(base_addr: u32) -> Option<Cmdweprotb>  {
    ...
    defmt::info!("wp bit calc: {:032b}, secnum {} , group8 {}", inverse, secnum, group8);
...
impl FlashController {
    pub fn new(controller: flashctl::Flashctl) -> Self {
        FlashController {
            flash: controller
        }
    }
...
/*
fn flashctl_write(addr: u32) -> bool {
    // this was scary and wrong!
    // each bit starting from 4 and going up protects 8 sectors starting from 32kb
    // really, each bit protects 8 sectors, but the bottom 4 bits are ignored / should use weprota.
    let wp = calc_wp_bits_8k(addr).unwrap();

    FLASHCTL.cmdweprotb().write_value(wp);
    FLASHCTL.cmdaddr().write_value(addr);
    ...
    info!("status: {:032b}", FLASHCTL.statcmd().read().0);
    if FLASHCTL.statcmd().read().cmdpass().to_bits() == 0 {
        return false;
    } else {
        return true;
    }
...
fn flashctl_erase() {
    let wp = calc_wp_bits_8k(0x0001_E000).unwrap();
    FLASHCTL.cmdweprotb().write_value(wp);
    ...
    FLASHCTL.cmdbyten().write_value(Cmdbyten(0x1FF)); // DS is wrong - actually a 9 bit value not 8 bit.
    FLASHCTL.cmdtype().write(|w| {
        w.set_command(flashctl::vals::Command::ERASE);
        w.set_size(flashctl::vals::Size::SECTOR);
    });
